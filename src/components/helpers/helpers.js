...
const getTime = ms => {
  ...
  const retSeconds =
    seconds.toString().length > 1 && seconds % 60 !== 0 && seconds - minutes * 60 >= 10
      ? seconds - minutes * 60
      : seconds % 60 === 0
      ? '00'
      : `0${seconds - minutes * 60}`;
  const retMinutes =
    minutes.toString().length > 1 && minutes % 60 !== 0 && minutes - hours * 60 >= 10
      ? minutes - hours * 60
      : minutes % 60 === 0
      ? '00'
      : `0${minutes - hours * 60}`;
...
export { questionNumber, answerLetter, getTime, getStatus, getCategoryName };
